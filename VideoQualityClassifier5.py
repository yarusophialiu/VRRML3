...
class NaturalSceneClassification(ImageClassificationBase):
    def __init__(self, num_framerates, num_resolutions, num_patch):
        super().__init__()
        self.network = nn.Sequential(
            nn.Conv2d(3 * num_patch, 32, kernel_size = 3, padding = 1),
            nn.ReLU(), 
...
# input image 64x64, consecutive decocded patches, stack 2 or 3 consecutive patches
# no resolution, maybe add resolution?
if __name__ == "__main__":
    ...
    data_train_directory = f'{data_directory}/train_consecutive_dec/train' 
    data_test_directory = f'{data_directory}/test_consecutive_dec/test'
    SAVE_MODEL = False
    ...
    NUM_PATCH = 3
    TYPE = "consecutive_dec"

    dataset = ConsecutivePatchDataset(directory=data_train_directory, NUM_PATCH=NUM_PATCH) # len 19720
    test_dataset = ConsecutivePatchDataset(directory=data_test_directory, NUM_PATCH=NUM_PATCH) 
    ...
    if START_TRAINING:
        ...
        model = NaturalSceneClassification(num_framerates, num_resolutions, NUM_PATCH)
        # print(model)
